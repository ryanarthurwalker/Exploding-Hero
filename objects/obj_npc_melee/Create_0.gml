image_xscale = .2;
image_yscale = .2;

// Movement and behavior properties
attack_range = 40; // Range for melee attack
attack_timer = 0; // Timer for attack cooldown
attack_cooldown = 60; // Cooldown time in frames (1 second if game runs at 60 FPS)

// Sword attack properties
is_attacking = false; // Is the NPC currently attacking?
attack_duration = 15; // Duration of the attack animation
attack_timer_active = false; // Flag for attack animation timer